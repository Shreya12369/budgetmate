    ...
    if choice == "Login":
        ...
        if st.button("Login"):
            ...
            if user:
                ...
                st.session_state.page = "Dashboard"  # ✅ optional, to redirect properly
                st.rerun()  # ✅ force full refresh with session data
            else:
    ...
    # view goals, create and manage savings goal
        ...
        with st.form("manual_saving"):
            ...
            if submitted:
                ...
                conn.commit()
                st.success(f"£{manual_amount:.2f} added as manual saving.")
                st.rerun()

        # always recalculate remaining for past months and current month on last day
        selected_month_str = datetime.today().strftime("%Y-%m")
        last_day = monthrange(today.year, today.month)[1]

        # loop through all months in the budgets table for this user
        c.execute("SELECT month FROM budgets WHERE user_id=?", (st.session_state.user[0],))
        all_months = [row[0] for row in c.fetchall()]

        for month in all_months:
            budget_month = datetime.strptime(month, "%Y-%m")
            month_year = (budget_month.month, budget_month.year)

            is_last_day_of_this_month = (
                    today.year == month_year[1] and today.month == month_year[0] and today.day ==
                    monthrange(today.year, today.month)[1]
            )

            is_past_month = budget_month < today.replace(day=1)

            # only run if it's a past month, or it's the current month and today is the last day
            if is_past_month or is_last_day_of_this_month:
                m, y = f"{month_year[0]:02d}", str(month_year[1])

                # get total expense for that month
                c.execute(
                    "SELECT amount FROM transactions WHERE user_id=? AND strftime('%m', date)=? AND strftime('%Y', date)=? AND type='Expense'",
                    (st.session_state.user[0], m, y))
                expenses = sum([row[0] for row in c.fetchall()])

                # get that month's budget
                c.execute("SELECT budget FROM budgets WHERE user_id=? AND month=?", (st.session_state.user[0], month))
                result = c.fetchone()
                ...
                if remaining > 0:
                    #check if already in savings
                    c.execute("SELECT id FROM savings WHERE user_id=? AND month=? AND year=? AND source='remaining'",
                              (st.session_state.user[0], m, y))
                    existing = c.fetchone()

                    if existing:
                        c.execute("UPDATE savings SET amount=? WHERE id=?", (remaining, existing[0]))
                    else:
                        c.execute(
                            "INSERT INTO savings (user_id, month, year, amount, source) VALUES (?, ?, ?, ?, 'remaining')",
                            (st.session_state.user[0], m, y, remaining))
        conn.commit()
        ...
        # fetch all savings entries again for selection
        c.execute(
            "SELECT id, month || '/' || year AS Period, amount, source FROM savings WHERE user_id=? ORDER BY id DESC",
            (st.session_state.user[0],))
